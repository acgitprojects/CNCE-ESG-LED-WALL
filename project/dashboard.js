/* ============================================================
   CNCB ESG Dashboard — data, i18n, icons, render, motion, floors
   ============================================================ */
(function () {
  "use strict";
  ...
  // combined HQ aggregate: average the environment, sum the savings
  ...
  // RESET-style star ratings derived from values (lower = better)
  ...
  const ARC = Math.PI * 250; // gauge arc length ≈ 785
  ...
  function applyFloorStatic(f) {
    // text/markup that changes per floor (not the count-up itself)
    const st = stage();
    const r = rating(f.gauge);
  ...
  function setRotateMs(ms) { ctrl.rotateMs = ms; if (ctrl.mode === "rotate") startRotate(); }
  ...
  function mount() {
    ...
    setActiveTab(0);
    primeFinal();   // always-visible fallback
    fit();
    animate();
  }
  ...
  window.CNCB = { mount, fit, replay, setMode, gotoFloor, setRotateMs };
